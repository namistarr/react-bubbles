...
A: Tokens are generated by a backend server when correct credentials are received to authenticate them. 

- [ x ] What steps can you take in your web apps to keep your data secure?
A: Protected routes, requiring authentication.

- [ x ] Describe how web servers work.
A: 1. Computer connects to internet 
   2. The web browser obtains an the IP address of the domain name 
   3. The server sends the browser the requested data

- [ x ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
A: C: create(post) R: read(get) U: update(put) D: delete
